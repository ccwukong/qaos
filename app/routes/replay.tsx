          ...
          try {
            ...
            if (event.type === "step_start") {
              ...
              setMessages(prev => prev.map(m => 
                m.id === `prompt-${stepIdx}` ? { ...m, status: "running" } : m
              ));
            ...
            if (activeAgentMessageId && (event.type === "thought" || event.type === "status" || event.type === "step" || event.type === "error")) {
              const prefix = event.type === "error" ? "❌ " : event.type === "step" ? "🎯 " : event.type === "status" ? "ℹ️ " : "";
              activeAgentContent += (activeAgentContent === "..." ? "" : "\n") + prefix + event.data;
              
              setMessages(prev => prev.map(m => 
                m.id === activeAgentMessageId 
                  ? { ...m, content: activeAgentContent, status: event.type === "error" ? "error" : "running" } 
                  : m
              ));
            }
            ...
            if (event.type === "done") {
               ...
               setMessages(prev => prev.map(m => m.status === "running" ? { ...m, status: "done" } : m));
            }
      ...
      <div className="chat-panel">
        <div className="chat-header">
          ...
          <span className="chat-header-title">
            {testCase ? `🔄 ${testCase.name} (${testCase.mode === "test" ? "Smart Replay" : "Classic Replay"})` : "🔄 Replay Session"}
          </span>
        </div>
          ...
          {!testCase && (
            <div className="chat-empty">
              ...
              <button
                ...
                className="send-btn"
                style={{ width: "auto", padding: "10px 24px", fontSize: "14px", borderRadius: "8px", cursor: "pointer" }}
              >
                📂 Load JSON File
              </button>
            </div>
