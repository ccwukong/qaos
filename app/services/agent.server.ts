...
- Use "type_secret" to securely type a value from .env (e.g. key="TEST_PASSWORD"). You MUST provide exact x,y coordinates for the input. DO NOT use 'text' field for this action.
...
export async function reason(
  ...
  const messages: Array<{ role: "user" | "assistant"; content: string | Array<{ type: string; text?: string; image?: string; mimeType?: string }> }> = [];
  ...
  try {
    ...
    if (status === 429) {
      return { action: "error" as const, message: "Rate limited by LLM provider. Please wait a moment and try again.", reasoning: msg };
    }
    if (status === 401 || status === 403) {
      return { action: "error" as const, message: `Invalid API key for ${provider}. Check Settings → API Keys.`, reasoning: msg };
    }
    return { action: "error" as const, message: `LLM request failed (${provider}): ${msg}`, reasoning: "API call threw an exception" };
  }
...
`;
  ...
  try {
    ...
    return { action: "proceed", reasoning: "Could not parse classifier output, defaulting to proceed." };
  } catch (err) {
