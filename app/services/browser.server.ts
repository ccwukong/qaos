...
class BrowserManager {
  ...
  async getPage(sessionId: string, url?: string, headless: boolean = true): Promise<Page> {
    console.log(`[BrowserManager] getPage called for ${sessionId}. Url: ${url}, Headless: ${headless}`);
    let session = this.sessions.get(sessionId);
    let capturedUrl: string | undefined = undefined;

    // If session exists but headless mode mismatch, close it (restart)
    if (session && session.headless !== headless) {
      console.log(`[BrowserManager] Headless mode changed (${session.headless} -> ${headless}). Restarting session ${sessionId}.`);
      try {
    ...
    let browser: Browser;
    try {
      console.log(`[BrowserManager] Launching new browser (Stealth). Headless: ${headless}`);
      ...
      throw new Error(`Failed to launch browser. Is Chromium installed? Run "npx puppeteer browsers install chrome". Details: ${msg}`);
    }
    ...
    session = { browser, page, consoleErrors, lastActivity: Date.now(), headless };
  ...
  async executeAction(
    sessionId: string,
    action: { action: string; x?: number; y?: number; text?: string; direction?: string; url?: string },
    ...
    if (!session && action.action === "navigate" && action.url) {
      console.log(`[BrowserManager] Lazy init session ${sessionId} for navigation to ${action.url}. Headless: ${headless}`);
    ...
    if (session.page.isClosed()) {
      this.sessions.delete(sessionId);
      throw new Error("Browser session was closed. Please start a new session or navigate to a URL.");
    }
    ...
    try {
      ...
      if (msg.includes("detached Frame") || msg.includes("Target closed") || msg.includes("Session closed")) {
        this.sessions.delete(sessionId);
        throw new Error("Browser session lost (detached frame). Please navigate to a URL to restart.");
      }
