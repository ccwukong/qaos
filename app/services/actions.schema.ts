...
export const ActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    ...
    key: z.string().describe("The .env variable name (e.g. TEST_USER)"),
...
export function getActionDescriptions(): string {
  ...
  actions.push(`- {"action":"navigate","url":"<full_url>","reasoning":"<why>"} — Navigate to a new URL`);
  actions.push(`- {"action":"click","x":<num>,"y":<num>,"reasoning":"<why>"} — Click at x,y coordinates`);
  actions.push(`- {"action":"type","x":<num>,"y":<num>,"text":"<text>","reasoning":"<why>"} — Type text`);
  actions.push(`- {"action":"scroll","direction":"up"|"down","reasoning":"<why>"} — Scroll page`);
  actions.push(`- {"action":"ask_human","question":"<question>","reasoning":"<why>"} — Ask user for help`);
  ...
  actions.push(`- {"action":"use_skill","skill_name":"<name>","reasoning":"<why>"} — Invoke a skill`);
  actions.push(`- {"action":"type_secret","x":<num>,"y":<num>,"key":"<ENV_VAR>","reasoning":"<why>"} — Securely type secret from .env (NEVER use 'text')`);
  actions.push(`- {"action":"run_script","skill_name":"<name>","script":"<script>","args":{...},"reasoning":"<why>"} — Run skill script`);
